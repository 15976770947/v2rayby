...
startlotserver(){
	...
	bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/15976770947/v2rayby/main/Install.sh) install
...
remove_all(){
	...
	if [[ -e /appex/bin/lotServer.sh ]]; then
		bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/15976770947/v2rayby/main/Install.sh) uninstall
	fi
	clear
	echo -e "${Info}:清除加速完成。"
	sleep 1s
